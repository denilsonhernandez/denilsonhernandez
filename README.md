Hi, I'm Denilson Hernandez 👋
Electronics & Automation Engineer

Electronics and Automation Engineer with 3 years of experience in technical support, electrical systems, automation, component analysis, and troubleshooting.

I enjoy working at the intersection of electronics, embedded systems, automation, and hardware, combining software and physical systems to solve engineering problems.

📍 Burlington, Ontario, Canada
📧 deni182001@gmail.com
💼 LinkedIn

👨‍💻 About Me
🎓 Bachelor's Degree in Electronic and Automation Engineering from Universidad Autónoma de Nuevo León (UANL)
⚡ Experience supporting Electric Arc Furnace systems, 3-phase power systems, automation controllers, and industrial instrumentation
🔧 Hands-on experience with electrical troubleshooting, sensors, actuators, microcontrollers, and embedded systems
🏭 Experience with PLC programming, Ladder Logic, Studio 5000, HMI systems, and industrial communication networks
💻 Programming experience with C/C++
📊 Experience creating technical reports and engineering analysis using Power BI
🌎 Fluent in English and native Spanish speaker
🛠 Technical Skills
Embedded Systems & Electronics

STM32 Arduino Embedded C C++ GPIO ADC RTC Interrupts Sensors 7-Segment Displays

Automation & Controls

PLC Programming Ladder Logic Studio 5000 HMI Alarm Logic Industrial Communication Networks

Electrical Systems

3-Phase Power Systems Electrical Troubleshooting Industrial Instrumentation Electric Arc Furnace Systems

Engineering Tools

AutoCAD Oscilloscope Multimeter Power BI Microsoft Office

🚀 Featured Projects
⏱ STM32 Programmable Digital Clock

Embedded digital clock developed using an STM32 microcontroller featuring external interrupts, ADC input, RTC timekeeping, GPIO control, and multiplexed 7-segment displays.

Technologies:
STM32 Embedded C ADC RTC GPIO Interrupts Digital Electronics

🔗 View Project

🏭 Automated Color Sorting Conveyor

Arduino-based automated conveyor capable of detecting objects by color and automatically sorting them into different containers.

The project integrates color sensing, ultrasonic sensors, servo motors, stepper motor control, and automation logic.

Technologies:
Arduino C++ Sensors Servo Motors Stepper Motors Automation

🔗 View Project

🦾 Robotic Pick-and-Place Arm

Arduino-controlled robotic manipulator designed to perform an automated pick, transfer, and place sequence using multiple servo motors and stepper-controlled base rotation.

Technologies:
Arduino C++ Robotics Servo Control Stepper Motors Motion Control

🔗 View Project

🔢 STM32 Keypad & Display Interface

STM32-based embedded interface that decodes input from a 4x4 matrix keypad using external interrupts and displays entered numbers on an 8-digit multiplexed 7-segment display.

Technologies:
STM32 Embedded C GPIO External Interrupts Matrix Keypad Display Multiplexing

🔗 View Project

💼 Professional Experience
Remote Support Engineer — AMI Automation

Nov 2024 – Jun 2026

Provided remote technical support for Electric Arc Furnace systems across five plants in the United States and Canada
Diagnosed faults in 3-phase power systems and automation controllers
Contributed to maintaining approximately 95% system yield
Helped reduce unplanned downtime by more than 20 hours per month through preventive remote monitoring
Integrated monitoring equipment and developed custom alarm logic for real-time variable tracking
Created technical performance reports using Power BI
Electronic Components Intern — Schneider Electric

Feb 2023 – Jun 2024

Supported the selection and configuration of electronic and electromechanical components
Collaborated with manufacturers and distributors for component sourcing and technical evaluation
Worked with CAD, sales, environmental, and engineering teams
Evaluated components according to technical, regulatory, environmental, and customer requirements
Supported engineering databases and process improvements using PDM tools
🎓 Education

Bachelor's Degree in Electronic and Automation Engineering
Universidad Autónoma de Nuevo León — 2024

📚 Professional Development
Industrial Control, Automation and Networking Program — Risoul RS
Industrial Automation – Allen-Bradley PLC — Ingeniería y Automatización Flores
🎯 Areas of Interest
Embedded Systems
Electronics Design
Hardware Development
Industrial Automation
Control Systems
Robotics
Electrical Systems
Hardware/Software Integration
📫 Let's Connect

I'm interested in connecting with engineers and teams working on electronics, automation, embedded systems, and hardware development.

LinkedIn • GitHub • Email
